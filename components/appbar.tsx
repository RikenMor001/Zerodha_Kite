    ...
    <div className="border-b shadow-4xl p-3 flex bg-neutral-900 border-b-neutral-800 justify-between items-center">
      ...
      <div className="relative flex flex-col items-center space-x-2 text-white">
        <div className="flex items-center space-x-2">
          <div className="bg-orange-500 rounded-full w-6 h-6 flex items-center justify-center text-sm">{userInitial}</div>
          <button className="text-sm hover:text-orange-500 hover:underline" onClick={() => setDropdownOpen(!dropdownOpen)}>{userId}</button>
        </div>
        {dropdownOpen && (
          <div className="absolute top-full mt-2 w-32 bg-neutral-800 text-white shadow-lg rounded-md py-2 z-10">
            <button className="block px-4 py-2 hover:bg-neutral-700 w-full text-left" onClick={handleLogout}>Logout</button>
          </div>
        )}
      </div>
    </div>
